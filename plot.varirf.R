plot.varirf1 = function (x, plot.type = c("multiple", "single"), names = NULL, 
          ...
          padj.mtext = NA, col.mtext = NA, ...) 
{
  ...
  dataplot <- function(x, iname) {
    ...
    if ((x$model == "varest") || (x$model == "vec2var")) {
      if (x$ortho) {
        text1 <- paste("Función Impulso Respuesta ortogonal desde", 
                       iname, sep = " ")
      }
      else {
        text1 <- paste("Función Impulso respuesta desde", iname, 
                       sep = " ")
      }
    }
    ...
    if (x$cumulative) 
      text1 <- paste(text1, "(Acumulada)", sep = " ")
    text2 <- ""
    if (x$boot) 
