...
Para determinar la presencia de un efecto umbral en la relación de integración se plantea el problema bajo las siguientes hipótesis: 
...
<<fig-2.4,echo=F,fig.cap='Evolución del logaritmo del precio en supermercado de la palta, 2008-2016\\label{fig5.4}', out.width='4.5in', out.height='3.5in', fig.align='center'>>=
par(family="serif", bty="l")
layout(matrix(c(1,1,1,1,1,
                2,2,0,3,3),ncol =5, byrow=TRUE))
...
<<graph5.7, echo=FALSE, fig.cap='Función acumulada de impulso respuesta\\label{graph5.7}'>>=
                                                             ...
                                                             "ResBoot"), hpc = c("none", "foreach")) 
{
  ...
  if (is.null(fixed.beta)) {
    ...
    ve <- VECM(data, lag = lag, include = "const", beta = fixed.beta, 
               estim = "2OLS")
  }
      ...
      lmtest_with_resBoot <- function(ve) {
        ...
        if (is.null(fixed.beta)) {
          ve.boot <- VECM(data.boot, lag = lag, include = "const", 
                          estim = "ML",beta0 = rep(1,dim(data)[1,]))
        }
        else {
          ve.boot <- VECM(data.boot, lag = lag, include = "const", 
...
hansen2 = TVECM_rest(datos, lag=8, ngridTh = 100, trim=0.05, nboot=100,
fixed.beta =NULL)
...
mono = TVECM(datos, lag=8, nthresh = 1, trim=0.05, ngridBeta = 100, ngridTh = 200, plot=TRUE, include=c("none"), common = "All", beta = list(int = c(-0.5,1.5),beta0 = rep(1,nrow(datos))))
