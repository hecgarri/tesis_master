...
<<fig-2.3,echo=F,fig.cap='Evolución del logaritmo del precio mayorista de la palta, 2008-2016\\label{fig5.1}', out.width='4.5in', out.height='3.5in', fig.align='center'>>=
par(family="serif", bty="l")
layout(matrix(c(1,1,1,1,1,
                2,2,0,3,3),ncol = 5, byrow=TRUE))
...
<<out.width='5in', out.height='3.5in',fig.cap="Inclusión de variables escalón\\label{fig5.5}">>=
...
<<>>=
modelo = lapply(1:9, function(x) VAR(datos, p=x, type = "const",
                                     exogen = dummy))
...
modelito = VAR(datos, p=9, exogen = dummy)
@
...
normality(modelito)
serial.test(modelito, lags.pt = 13, type = c("PT.adjusted"))
@
