...
\caption{Evolución del logaritmo del precio mayorista de la palta, 2008-2016\label{fig5.1}}\label{fig:fig-2.3}
\end{figure}
...
Antes de la determinación del rango de cointegración del proceso, es necesario tener en cuenta que el uso del contraste de cointegración de Johansen que se utilizará para dichos fines es sensible a la especificación del modelo vectorial de corrección del equilibrio que subyace. Para la especificación de dicho modelo recurrió a diferentes criterios. 

En primer lugar, una inspección visual de ambas series revela la presencia de un comportamiento en forma de pulso\footnote{Esta observación surge de la participación del autor en el encuentro anual de la Sociedad Chilena de Estudios Regionales, SOCHER} el cual puede apreciarse en la figura \ref{fig5.5}. Para modelar dicho comportamiento se procedió a crear dos variable mudas de acuerdo a los siguientes criterios: 

\begin{align}
\text{Pulso}_{1} = & I(t\geq 312) = \begin{cases} 1 & t\geq 312 \\ 0 & \text{en otro caso} \end{cases} \\ 
\text{Pulso}_{2} = & I(t\geq 76 \quad \& \quad t\leq 122) = \begin{cases} 1 & t\geq 76 \quad \& \quad t\leq 122 \\ 0 & \text{en otro caso} \end{cases} 
\end{align}
...
\caption{Inclusión de variables escalón\label{fig5.5}}\label{fig:unnamed-chunk-12}
\end{figure}


\end{knitrout}

Otro aspecto a destacar es la distancia más o menos constantes existente entre ambas series de precios, por lo que se asumirá que la relación de cointegración entre ambas series contiene una constante como término determinístico. 

El número de rezagos a incluir en el modelo se determinó con el objetivo de prevenir problemas de autocorrelación. De esta manera, se estimó el modelo secuencialmente utiliando entre 2 y  12 rezagos y se calculó el estadístico de Ljung-Box multivariado para los residuos rezagados entre el orden mínimo posible y aquellos de orden 52. Los p-valores correspondientes a cada uno de los contrastes están resumidos en la figura \ref{fig5.6}. De esta manera, se aprecia que la hipótesis nula de independencia no se puede rechazar en la mayoría de los casos para el modelo con ocho términos rezagados. 

\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{figure}[H]

{\centering \includegraphics[width=5in,height=3.5in]{figure/unnamed-chunk-13-1} 

}

\caption{Número de Rezagos para el contraste de Independencia\label{fig5.6}}\label{fig:unnamed-chunk-13}
\end{figure}
...
\begin{equation}
\Delta \mathbf{y}_{t} = \boldsymbol{\Pi}\mathbf{y}_{t-1}+
\end{equation}
...
\\
\midrule
$r \leq 1$  & \multicolumn{2}{c}{ 18.48}  & 7.52 & 9.24 & 12.97\\
$r = 0$     & \multicolumn{2}{c}{ 48.58}  & 17.85 & 19.96 & 24.60\\
\bottomrule
\end{tabular}
\end{center}
\label{tab-10}
...
\midrule
$r \leq 1$ & \multicolumn{2}{c}{ 18.48} & 7.52 & 9.24 & 12.97 \\
$r = 0$ & \multicolumn{2}{c}{30.10} & 13.75 & 15.67 & 20.20 \\
\bottomrule
\end{tabular}
\end{center}
\label{tab-11}
...
Para especificar el modelo VAR más adecuado se procede a utilizar los siguientes criterios de información

\begin{table}
\centering
\caption{Selección del número de rezagos}
\begin{tabular}{@{}llll@{}}
\toprule \\
AIC & HQ & SC & FPE \\
\midrule \\ 
...
{\centering \includegraphics[width=4.5in,height=3.5in]{figure/unnamed-chunk-18-1} 
...
{\centering \includegraphics[width=4.5in,height=3.5in]{figure/unnamed-chunk-18-2} 
...
{\centering \includegraphics[width=4.5in,height=3.5in]{figure/unnamed-chunk-19-2} 
